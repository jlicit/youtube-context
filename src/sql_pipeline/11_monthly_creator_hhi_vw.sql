CREATE OR REPLACE VIEW `{{ project }}.{{ dataset }}.monthly_creator_hhi_vw` AS
WITH events AS (
  SELECT
...
shares AS (
  SELECT
    ...
    SAFE_DIVIDE(
      minutes_content,
      NULLIF(SUM(minutes_content) OVER (PARTITION BY month_start), 0)
    ) AS share
  FROM per_creator
),
agg AS (
...
top AS (
  ...
  FROM (
    SELECT
      month_start, creator, share,
      ROW_NUMBER() OVER (PARTITION BY month_start ORDER BY share DESC, creator ASC) AS rn
    FROM shares
  )
  WHERE rn = 1
)
SELECT
  ...
  ROUND(
    SAFE_DIVIDE(a.entropy_nats, LN(NULLIF(a.creator_count, 1))),
    6
  ) AS entropy_norm_0_1,
  ROUND(
    SAFE_DIVIDE(
